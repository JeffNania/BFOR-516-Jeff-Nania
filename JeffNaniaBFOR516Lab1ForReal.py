...
cr = ['Albany', 'Columbia', 'Fulton', 'Greene', 'Montgomery', 'Rensselaer', 'Saratoga', 'Schenectady', 'Schoharie', 'Warren', 'Washington']
...
total_deaths_plot = alb.groupby(['date', 'county'])['deaths'].sum().unstack().plot(logy=False, figsize=(10,5))
total_deaths_plot.set(xlabel='Date', ylabel='Number of Deaths', title=date_title)
total_deaths_plot.get_figure()

total_deaths_plot.get_figure().savefig(output, bbox_inches='tight', dpi=300)

#Create a plot showing the total number of cases in the Capital Region.

total_cases = alb.groupby(['date'])['cases'].sum().plot()
print(total_cases)


total_cases_plot = cr.groupby(['date', 'county'])['cases'].sum().unstack().plot(logy=False, figsize=(10,5))
total_cases_plot.set(xlabel='Date', ylabel='Number of Cases', title=date_title)
